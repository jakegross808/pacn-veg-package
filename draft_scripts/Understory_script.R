...
ftpc_xy <- FilterPACNVeg("Events_extra_xy", plot_type = "Fixed", cycle = 3,
  ...
  mutate(lat = Start_Lat,
         long = dplyr::case_when(Start_Long > 0 ~ Start_Long * -1,
                                      TRUE ~ Start_Long)) %>%
  mutate(p_type = dplyr::case_when(Plot_Number < 16 ~ "Fixed",
                          TRUE ~ "Rotational"))
...
Tot_Cov <- Understory %>%
  ...
  dplyr::group_by(Cycle, Unit_Code, Sampling_Frame, Plot_Type, Plot_Number, Stratum) %>%
...
#........DENSITY PLOT ---- 
means <- Tot_Cov %>%
  dplyr::group_by(Cycle, Unit_Code, Sampling_Frame, Stratum) %>%
...
Tot_Cov %>%
  ...
  ggplot2::ggplot(aes(x = reorder_within(Plot_Number, desc(tot_pct_cov), Stratum), 
             y = tot_pct_cov, fill = Cycle)) +
  geom_col(position = position_dodge()) +
  scale_fill_brewer(palette="Accent") +
  facet_wrap(vars(Stratum), dir = "v", scales = "free_x") +
  ggplot2::ylab("Total % Cover") + ggplot2::xlab("Plot Number") +
  scale_x_reordered()
...
Tot_Cov %>%
  ...
  ggplot2::geom_point(aes(color = Plot_Number), size = 1, na.rm = TRUE) +
  facet_wrap(vars(Stratum), dir = "v", scales = "free_x") 
...
Tot_Cov_Chg <- Tot_Cov %>%
  ...
  mutate(chg3v1 = dplyr::case_when(!is.na(chg3v1) ~ chg3v1, 
         TRUE ~ chg3v2))


#........BAR CHG/PLOT# ----
Tot_Cov_Chg %>%
  mutate(direction = dplyr::case_when(chg3v1 > 0 ~ "Pos",
                               chg3v1 < 0 ~ "Neg" )) %>%
  ggplot2::ggplot(aes(x = reorder_within(Plot_Number, desc(chg3v1), Stratum), 
             y = chg3v1, fill = direction)) +
  geom_col(position = position_dodge()) +
  ggplot2::scale_fill_manual(values = c("#CC0000", "#009900")) +
  #scale_fill_brewer(palette="Accent") +
  #facet_grid(rows = vars(Stratum), cols = vars(Sampling_Frame)) +
  facet_wrap(vars(Stratum), dir = "v", scales = "free_x") +
  scale_x_reordered() +
...
Tot_Cov_Chg_range <- Tot_Cov_Chg %>%
  dplyr::group_by(Sampling_Frame, Stratum) %>%
...
#........BAR YEARLY MEANS----
Tot_Cov_Stats %>%
  ggplot2::ggplot(aes(x = PARAM, y = MEAN, fill = PARAM)) +
  geom_col(position = position_dodge()) +
...
Tot_Cov_Stats %>%
  ...
  geom_errorbar(aes(ymin=L, ymax=R), width=.2,
                position=position_dodge(.9)) +
  ggplot2::labs(y = "Change in Total % Cover") +
...
# Tot_Cov_Stats %>%
#   filter(Cycle == '2') %>%
#   mutate(highlight = dplyr::case_when(Unit_Code == "AMME" ~ 'AMME',
#                                Unit_Code != "AMME" ~ 'Other')) %>%
#   ggplot2::ggplot(aes(x = reorder_within(Sampling_Frame, -MEAN, Stratum), 
#              y = MEAN, 
#              fill = highlight)) +
#   geom_bar(stat="identity") + #, position = position_dodge()
#   geom_errorbar(aes(ymin=L, ymax=R), width=.2) + #,position=position_dodge(.9)
#   ggplot2::labs(x = "Sampling Frame", y = "% Cover") +  
#   scale_x_reordered() +
#   ggplot2::theme(axis.text.x=element_text(angle = 90, hjust = 1, vjust = 0.33)) +
#   guides(fill=FALSE) +
#   ggplot2::scale_fill_manual(values = c("#FF3300", "#736F6E")) +
...
Nat_Cov <- Understory %>%
  ...
  dplyr::group_by(Cycle, Unit_Code, Sampling_Frame, Plot_Type, Plot_Number, Stratum, Nativity) %>%
  #Total hits at each point for each Stratum for entire plot 
  # (can be > 300 points or >100%)
...
Nat_Cov %>%
  ...
  ggplot2::ggplot(aes(x = reorder_within(Plot_Number, -tot_pct_cov, within = Nativity), 
             y = tot_pct_cov, fill = Cycle)) +
  geom_bar(stat="identity", position = position_dodge()) +
  scale_fill_brewer(palette="Accent") +
  facet_grid(rows = vars(Stratum), cols = vars(Nativity), scales = "free") +
  scale_x_reordered() +
  ggplot2::xlab("Plot Number")
...
Nat_Cov_Chg <- Nat_Cov %>%
  ...
  complete(tidyr::nesting(Cycle, Unit_Code, Sampling_Frame, Plot_Type, Plot_Number), 
           tidyr::nesting(Nativity, Stratum),  
...
Nat_Cov_Chg %>%
  ...
  ggplot2::ggplot(aes(x = Plot_Number, y = chg3v1, fill = Nativity)) +
  geom_col(position = position_dodge()) +
  facet_wrap(~ Stratum + Nativity, scales = "free_x") +
  ggplot2::scale_fill_manual(values = nativity_colors) +
  scale_x_reordered() +
  ggplot2::xlab("Plot Number") + ggplot2::ylab("Change in % Cover")
...
Nat_Rich_no_unks %>%
  filter(Plot_Type  == "Fixed") %>%
  ggplot2::ggplot(aes(x=Cycle, y=richness, group=Plot_Number)) +
  geom_line(size=1, alpha=0.5, position=position_dodge(width=0.2)) +
  ggplot2::geom_point(position=position_dodge(width=0.2)) +
  ggplot2::xlab('Sample Cycle') +
  ggplot2::ylab('Understory Richness') +
  theme_bw() +
...
#  ........STRIP CHRT CHG----
Nat_Rich_Chg %>%
  ggplot2::ggplot(aes(x = Sampling_Frame, y=richness_chg)) +
  geom_jitter(width = 0.05) +
  ggplot2::ylab('Change in richness') +
  ggplot2::theme(axis.title.x=ggplot2::element_blank(),
        axis.text.x=ggplot2::element_blank(),
        axis.ticks.x=ggplot2::element_blank()) +
...
Nat_Rich_Chg %>%
  ...
  ggplot2::ggplot(aes(x = Native, y = `Non-Native`)) +
  ...
  ggplot2::ylab("Change in Non-Native Richness") +
  ggplot2::xlab("Change in Native Richness") 
...
nrs <- Nat_Rich_Stats %>%
  ...
  geom_errorbar(aes(ymin=L, ymax=R), width=.2,
                position=position_dodge(.9)) +
  ggplot2::labs(y = "Change in richness") +
  facet_wrap(vars(fct_rev(Stratum))) +
  scale_fill_brewer(palette="Dark2") +
  ggplot2::theme(axis.title.x=ggplot2::element_blank(),
        axis.text.x=ggplot2::element_blank(),
...
#  ........ BAR COV/PLOT----
Forms_Cov %>%
  ggplot2::ggplot(aes(x = reorder_within(Plot_Number, -tot_pct_cov, within = NLF), 
             y = tot_pct_cov, fill = Cycle)) +
  geom_bar(stat="identity", position = position_dodge()) +
  scale_fill_brewer(palette="Accent") +
...
Forms_Cov_Complete <- Forms_Cov %>%
  dplyr::ungroup() %>%
  #group_by("Unit_Code", "Sampling_Frame","Plot_Number","Nativity") %>%
  complete(Cycle, tidyr::nesting(Unit_Code, Sampling_Frame, Plot_Number, Stratum, 
                            Nativity, Life_form, NLF),  
           fill = list(tot_pct_cov = 0)) 
...
Forms_Cov_Complete_1a2 <- Forms_Cov_1a2 %>%
  dplyr::ungroup() %>%
  #group_by("Unit_Code", "Sampling_Frame","Plot_Number","Nativity") %>%
  complete(Cycle, tidyr::nesting(Unit_Code, Sampling_Frame, Plot_Number, 
                            Nativity, Life_form, NLF),  
           fill = list(tot_pct_cov = 0)) 
...
Forms_Cov_Chg %>%
  ...
  ggplot2::ggplot(aes(x = Plot_Number, y = tot_pct_cov_chg, fill = Nativity)) +
  geom_col(position = position_dodge()) +
...
Forms_Cov_Chg %>%
  #filter(Plot_Number %in% c(2,4,6,9)) %>%
  ggplot2::ggplot(aes(x =Sampling_Frame, y = tot_pct_cov_chg, color=Nativity)) +
  geom_jitter(width = 0.05) +
  ggplot2::geom_hline(yintercept=0, linetype = "dashed", color = "gray", size = 1) +
...
Forms_Cov_Complete %>%
  ...
  ggplot2::ggplot(aes(x=Cycle, y=tot_pct_cov, 
             group=interaction(Plot_Number, Nativity), 
             color = Nativity)) +
  geom_line(size=1, alpha=0.5, position=position_dodge(width=0.2)) +
  ggplot2::geom_point(position=position_dodge(width=0.2)) +
  ggplot2::xlab('Sample Cycle') +
  ggplot2::ylab('Total % Cover') +
  scale_color_brewer(palette="Dark2") +
...
fcs.means1a2 <- Forms_Cov_Stats_1a2 %>%
  ...
  mutate(ERR = dplyr::case_when(NPLOTS < 4 ~ NA_real_ , TRUE ~ ERR)) %>%
  droplevels() 
...
Forms_Cov_Chg_1a2 %>%
  ...
  ggplot2::ggplot(aes(x = reorder(NLF, tot_pct_cov_chg), y = tot_pct_cov_chg, fill = Nativity)) +
  ...
  ggplot2::labs(title="", y="Change in % Cover", x="Nativity + Lifeform") +
  scale_fill_brewer(palette="Dark2") +
...
Spp_Cov <- Spp_Cov_Filter %>%
  ...
  # group by plot (i.e. remove Transect and Point from grouping variable)
  dplyr::group_by(Cycle, Unit_Code, Sampling_Frame, Plot_Number, Stratum, 
...
Spp_Cov_Chg %>%
  droplevels() %>%
  ggplot2::ggplot(aes(x =Sampling_Frame, y = pct_cov_sp_chg, color=Name)) +
  geom_jitter(width = 0.05) +
  ggplot2::geom_hline(yintercept=0, linetype = "dashed", color = "gray", size = 1) +
  stat_summary(fun = mean, geom = "point", shape = 95, size = 8, color = "red") +
  facet_grid(vars(Stratum), vars(Life_form)) 
...
Spp_Cov %>%
  #mutate(Status = fct_rev(Status)) %>%
  ggplot2::ggplot(aes(x=Cycle, y=pct_cov_sp, 
...
p <- Spp_Cov_Chg %>%
  ...
  ggplot2::ggplot(aes(x = reorder(Name, pct_cov_sp_chg), y = pct_cov_sp_chg, fill = Nativity)) +
  ggplot2::geom_point(
    position = position_jitterdodge(jitter.width = 0.1, jitter.height = 0, dodge.width = 0.8),
    shape = 21, size = 1, alpha = 0.5) +
  ...
  ggplot2::geom_hline(yintercept=0, linetype="solid", 
             color = "black", size=0.5) +
  ggplot2::labs(title="", y="Change in % Cover", x="Species") +
...
tree.plot <- ggplot2::ggplot(Spp_Hits, aes(area = tot_hits, subgroup = Nativity, 
...
tree.plot2 <- ggplot2::ggplot(Spp_Hits, aes(area = tot_hits, subgroup = Nativity, 
...
#   filter(Cycle == "2") %>%
#   filter(Stratum == "UNDERSTORY1") %>%
#   complete(Unit_Code, Sampling_Frame, Plot_Number, tidyr::nesting(Code, Nativity),
#            fill = list(pct_cov_sp = 0)) %>%
#   dplyr::group_by(Unit_Code, Sampling_Frame, Code, Nativity) %>%
#   summarise(mean_cov_sp = mean(pct_cov_sp),
#             n = n()) %>%
#   ggplot2::ggplot(aes(area = mean_cov_sp, fill = Nativity, label = Code)) +
#   geom_treemap() +
...
#........JITTER PLOT SPP  ----
Spp_Cov_Chg %>%
  mutate(code_lab = dplyr::case_when(pct_cov_sp_chg <= -5 ~ Code,
                              pct_cov_sp_chg >= 5 ~ Code,
                              TRUE ~ "")) %>%
  filter(pct_cov_sp_chg != 0) %>%
  ggplot2::ggplot(aes(x = Plot_Number, y=pct_cov_sp_chg, 
...
Spp_Slope <- Spp_Cov_Chg %>%
  ...
  mutate(Understory = stringr::str_sub(Stratum,-1,-1)) %>%
  mutate(Direction = dplyr::case_when(pct_cov_sp_chg < 0 ~ "DECREASE",
                               pct_cov_sp_chg >= 0  ~ "INCREASE")) %>%
  mutate(code_lab = dplyr::case_when(`1` >= 5 ~ Code,
                              `2` >= 5 ~ Code,
                              TRUE ~ "")) %>%
...
ggplot(Spp_Slope) +
  ggplot2::geom_segment(aes(x=1, xend=2, y=`1`, yend=`2`, 
                   col=Nativity), size=.75, show.legend=T) + 
  geom_vline(xintercept=1, linetype="dashed", size=.1) + 
  geom_vline(xintercept=2, linetype="dashed", size=.1) +
  ggplot2::labs(x="", y="% Cover") +  
...
ggplot(Spp_Slope_X) +
  ...
  ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank()) +
  ggplot2::theme(axis.ticks = ggplot2::element_blank(),axis.text.x = ggplot2::element_blank()) +
  ggplot2::scale_color_manual(values = c("#CC0000", "#009900")) 

# ........BAR SP = X ----
sp.x <- "POAPRA"
Spp_Cov %>%
  filter(Code == sp.x) %>%
  ggplot2::ggplot(aes(x = reorder_within(Plot_Number, desc(pct_cov_sp), Stratum), y = pct_cov_sp, fill = Cycle)) +
  geom_col(position = position_dodge()) +
  ggplot2::ggtitle(sp.x) +
  scale_fill_brewer(palette="Accent") +
  facet_wrap(vars(Stratum), dir = "v", scales = "free_x") +
  ggplot2::ylab("Total % Cover") + ggplot2::xlab("Plot Number") +
  scale_x_reordered()
...
declines <- Spp_Cov_Stats %>%
  filter(Cycle == "CHG") %>%
  mutate(
    chg_dir = dplyr::case_when(
      MEAN > 0 ~ "Increase",
      MEAN < 0 ~ "Decrease",
      MEAN == 0 ~ "No Change")) %>%
  dplyr::group_by(Unit_Code, Sampling_Frame, Stratum, Nativity, chg_dir) %>%
  dplyr::summarize(chg_dir_count = n()) %>%
...
Spp_Pres <- Cover %>%
  ...
  complete(Cycle, Unit_Code, Sampling_Frame, Plot_Number, 
           tidyr::nesting(Nativity, Code, Name, Life_form),
           fill = list(present = 0)) %>%
  dplyr::group_by(Cycle, Unit_Code, Sampling_Frame,
           Nativity, Code, Name, Life_form) %>%
...
#........POINT CHG SPP ----
Spp_Pres_Chg %>%
  ggplot2::ggplot(aes(Sampling_Frame, prop_plts, colour = Nativity)) + 
  ...
  ggplot2::labs(title="Change in Landscape Frequency", y="Change in proportion of Plots", x="Sampling Frame", caption="(each point represents one species)") +
...
Spp_Pres_Chg %>%
  ...
  ggplot2::ggplot(aes(x = reorder(Name, presence_chg), presence_chg)) +   
  ...
  ggplot2::geom_hline(yintercept=0, linetype="solid", 
             color = "black", size=0.5) +
  scale_fill_brewer(palette="Dark2") +
  ggplot2::labs(title="", y="Plots (Change in Understory Presence)", x="Species", caption= paste('(',Spp_Pres_Chg$n[1],' paired plots)', sep='')) +
  guides(fill=guide_legend("")) +
  ggplot2::scale_y_continuous(breaks = seq(min(Spp_Pres_Chg$presence_chg),
