...
```{r avg-cover-bar, echo=FALSE, fig.cap= paste0("Average understory species cover across the monitoring area (fixed and rotational plots included) by 'Zone' and monitoring cycle. Zones represent disjointed geographic regions and were established to ensure well-interspersed plots across geographically complex plant communities. If only one colored 'Zone' appears in the overview map (Figure 2), then this dataset does not contain Zones and the 'Filter on Zone' box in this figure can be ignored. The 'No Veg' category represent the average percentage of the plot where no vegetation cover was recorded for any species."), message=FALSE, warning=FALSE, out.height="0px"}
# Dumb hack to get fig caption to show below plot + map
htmltools::span(class = "dummy-fig")
knitr::include_graphics(here("inst", "rmarkdown", "dummy_fig.PNG"))
```
...
*Native plant species ranked by greatest decrease in cover within a plot.*

::: vscroll-plot
```{r decline-natives, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=4.2}
decline_natives <- pacnvegetation::understory_spp_trends_rank(sample_frame = params$sample_frame, paired_change = TRUE, remove_nativity = "Non-Native", rank_by = "negative", top_n = 16) 
