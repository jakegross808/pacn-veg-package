...
The National Park's Pacific Island Inventory and Monitoring Network (PACN) monitors plant communities at the following locations:
```{r}
pacn_veg_locations <- FilterPACNVeg(data_name = "Presence") |>
  rename(`Park Code` = Unit_Code, `Plant Community` = Community, Location = Sampling_Frame_Formal) |>
  select(`Park Code`, `Plant Community`, Location) |>
  distinct() |>
  arrange(desc(`Plant Community`), `Park Code`) |>
  na.omit() 

knitr::kable(pacn_veg_locations, format = "html")
```
...
X fixed plots and X fixed transects were monitored in <Years> 
Numbers can be auto calculated here.
...
-   Paired change in cover between fixed plots indicated...
<Highlight total change in native vs non-native cover across all plots> (Figure \@ref(fig:bar-tot-cover-chg-graph)).
