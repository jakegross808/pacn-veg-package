...
```{r cover-composition-graph, echo=FALSE, fig.cap= "Average understory species cover across the monitoring area from the most recent monitoring event (2017).", message=FALSE, warning=FALSE}
und <- FilterPACNVeg("Understory", sample_frame = "Haleakala")

und <- und %>%
  mutate(Life_Form=replace(Life_Form, Code=="SOPCHR", "Shrub"))

# prep data for treemap
...
und2 <- UnderCombineStrata(und) %>%
  ...
  summarize(n = n(),
            ...
            Avg_Cover = mean(Plot_Percent),
            #Median = median(Plot_Percent),
            Std_Dev = sd(Plot_Percent),
            .groups = "drop")

library(treemapify)

tree.plot <- ggplot(und2, aes(area = Avg_Cover, subgroup = Nativity,
                                   subgroup2 = Life_Form, fill = Nativity, label = Code)) +
  geom_treemap(size = 1) +
...
```{r cover-composition-table, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r tot-cover-graph, echo=FALSE, fig.cap= "Native and non-native understory cover for the most recent monitoring (2017). Point numbers corrspond to fixed monitoring sites.", message=FALSE, warning=FALSE}
...
```{r chg-cover-graph, echo=FALSE, fig.cap= "Change in native vs. non-native cover from 2012-2017. Point numbers corrspond to fixed monitoring sites.", message=FALSE, warning=FALSE}
