...
bscols(sunburst_all, list(sunburst_by_grp, grp_filter))
```
...
sp_w_cover <- dplyr::filter(und2, Avg_Cover > 0)
cover_bar <- sp_w_cover %>%
  dplyr::mutate(Cycle = as.factor(Cycle)) %>%
  dplyr::filter(Avg_Cover > 0) %>%
  dplyr::group_by(GROUP_COL) %>%
  plotly::plot_ly(type = "bar", x = ~Cycle, y = ~Avg_Cover, color = ~GROUP_COL,
                    transforms = list(
                      list(type = "filter", 
                           target = ~Scientific_Name,
                           operation = 'in',
                           value = unique(sp_w_cover$Scientific_Name))
                    )) %>%
  plotly::layout(
    updatemenus = list(
      list(
        type = "dropdown",
        active = -1,
        buttons = lapply(unique(sp_w_cover$Scientific_Name), function(sci_name) {list(method = "restyle",
                                               args = list("transforms[0].value", sci_name),
                                               label = sci_name)})
      )
  ))
...
cover_bar
# 
# sp_filter <- filter_select("select-sp", "Select a species", und2_shared, ~Code, allLevels = FALSE)
# 
# bscols(cover_bar, sp_filter)
```
