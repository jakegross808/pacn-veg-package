...
ReadFTPC <- function(conn) {
  ...
  Events <- tbl_Events_short %>%
    ...
    dplyr::select(Unit_Code, Community, Sampling_Frame, Sampling_Frame_Formal, Year, Cycle, Plot_Type, Plot_Number,
  ...
  Events_extra_QAQC <- Events_extra %>%
    dplyr::select(Unit_Code, Sampling_Frame, Sampling_Frame_Formal, Start_Date, Year, Cycle, Plot_Type, Plot_Number,
  ...
  Events_extra_xy <- Events_extra %>%
    dplyr::select(Unit_Code, Sampling_Frame, Sampling_Frame_Formal, Year, Cycle, Plot_Type, Plot_Number,
  ...
  Events_extra_other <- Events_extra %>%
    dplyr::select(Event_ID, Unit_Code, Community, Sampling_Frame, Sampling_Frame_Formal, Year, Cycle, Plot_Type, QA_Plot,
...
#' }
FilterPACNVeg <- function(data_name, park, sample_frame, community, year, cycle, plot_type, plot_number, is_qa_plot, transect_type, species_code, sci_name, nativity, certified, verified, case_sensitive = FALSE, silent = FALSE) {
  ...
  all_filter_cols <- list(Unit_Code = ifelse(missing(park), NA, park),
                   ...
                   QA_Plot = if (missing(is_qa_plot)) {FALSE} else {is_qa_plot},
...
#' Read and return the species records from the PACN Veg Species Database
#' Vital Signs > 05_focal_terr_plant_communities > Data > Database > Veg_species_db
#'
#' @param db_paths Database path (local path after downloading from sharepoint)
#'
#' @return A tibble
#' @export
#'
...
#' pacnveg_master_spp_list <- read_spp_db(dbpath)
#'
#' }
read_spp_db <- function(db_paths) {
