...
ED_Database <- "C:/Users/JJGross/Documents/ArcGIS/Projects/ED/Early_Detection_20231109.gdb"
#C:/Users/JJGross/Documents/ArcGIS/Projects/ED/Early_Detection_20231109.gdb
Species_Database_Folder <- "C:/Users/JJGross/Documents/Databases_copied_local/Veg_species_db"
```
...
specimens <- ED_gdb |>
  ...
  separate(Photo_Taxon, c("Photo_Taxon_sp"), sep = "\\s*\\(.*", remove = FALSE) |>  
  ...
  mutate(New_sp_name = case_when(is.na(New_sp_name) & Taxon_LOC == 5 ~ Photo_Taxon_sp,
...
names <- readxl::read_xlsx(here::here("R", "PACN_veg_names.xlsx")) %>%
  select(Unit_Code, Park_Name, Island, State_Territory) %>%
  mutate(Unit_Code = str_trim(Unit_Code)) %>%
  distinct()

# Join formal names csv file and additional species info from PACN veg species database
specimens_sppDB <- specimens %>%
  left_join(names, by = join_by(Unit_Code == Unit_Code)) %>%
  left_join(raw_HAVO_spp_data, by = join_by(New_sp_name == Scientific_name)) 

# Prepare all columns for use in a herbarium label  
specimens_label_check <- specimens_sppDB %>%
  mutate(Title = paste(Park_Name, "Herbarium")) %>%
  #mutate(Island = paste(Island, State_Territory, sep = ", ")) %>% 
  mutate(Species = paste0("***", New_sp_name, "*** ", Authority)) %>%
  mutate(Family = Taxonomic_Family) %>%
  separate(Park_common_name, c("Common"), sep = ',', extra = "drop") %>%
  mutate(altitude = as.character(round(ESRIGNSS_ALTITUDE))) %>%
  mutate(altitude = replace_na(altitude, "")) %>%
  mutate(altitude = case_when(altitude != "" ~ paste(altitude, "m"),
                              .default = as.character(altitude))) %>%
  mutate(Coords = paste0("(",round(ESRIGNSS_LATITUDE, 6), ", ", 
                              round(ESRIGNSS_LONGITUDE, 6), ") ", 
                              altitude)) %>%
  mutate(Notes = replace_na(Xtra_Voucher_Notes, "")) %>%
  mutate(Project = paste("PACN I&M - Early Detection of Plant Species")) %>%
  mutate(Collector = Staff_List) %>%
  mutate(Col_Date = paste(day(created_date), month(created_date, label = TRUE), 
                          year(created_date), 
                          paste0(format(created_date, "%H:%M")))) %>%
  mutate(Det_Date = paste(day(Det_Date), month(Det_Date, label = TRUE), 
                          year(Det_Date))) %>%
  mutate(Coll = paste0(Staff_List, " (", Col_Date, ")")) %>%
  mutate(Det = paste0(Det, " (", Det_Date, ")")) %>%
  as_tibble() %>%
  select(Photo_Taxon, New_sp_name, Col_Date, Title, Island, Species, Family, Common, Locality, Coords, Habitat, Project, Coll, Det, Notes, created_date) 

# This is table that will be used to create the labels below
specimens_label_print <- specimens_label_check %>%
  select(-Photo_Taxon, -New_sp_name, -Col_Date) 

# Last check before pairing table down to just herbarium label components
spp_chklist <- specimens_label_check %>%
  select(created_date, Col_Date, Species) %>%
